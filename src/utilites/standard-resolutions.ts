...
export default function closestStandardForResolution (resolution: Resolution, method?: string): StdRes | null {
  if (!method) method = 'fill'
  let closestStd: StdRes | null = null
  common.forEach(std => {
    const wDiff = resolution.w - std.w
    const hDiff = resolution.h - std.h
    if (
      (method === 'fill' && (wDiff >= 0 && hDiff >= 0)) || // resolution bigger then standard
      (method === 'fit' && (wDiff <= 0 && hDiff <= 0)) // resolution smaller then standard
    ) {
      if (!closestStd) {
        closestStd = std
      } else if (wDiff <= resolution.w - closestStd.w || wDiff <= resolution.h - closestStd.h) {
        closestStd = std
      }
    }
  })
